...
- **未做**：不要为填满主榜加公开新闻源；AIHOT 不是订阅成员，禁止用昵称清归档。工作台过滤与调试包见工作台仓收尾报告。

## 死前端与雷达页面文案清理（2026-09-09，已部署并验收）

- **做了什么**：去掉页面已无 DOM 的伯乐精选、WaytoAGI 空壳、旧订阅管理器；统计条改为条目/未阅/来源；卡片只留作者和平台；小红书与微信栏目常驻。采集器、微信 `enabled: false`、归档未动。默认不跑的 `public.py` / `paid.py` 不当死代码删。
- **Git**：PR #49 合入 `master`，合并提交 `8fd21d2f`。页面脚本戳 `dead-ui-0909e`。
- **验收**：P5 `npx playwright test` **89 passed**。P6 用户口头通过并授权生产。GitHub Pages 已返回 `render-list.js?v=dead-ui-0909e`。
- **当前无活跃 SPEC/PLAN/TASK/TEST**。轻量清理，无四文件。
- **未做**：第二档采集器删除；工作台仍用的综合/高分/AI强相关按钮。

## 油管 RSS 抽风时重试并沿用上一轮条目（2026-09-08，已部署）

- **现象**：原生列表把油管「小岛大浪吹」写成没采到。快照 `16bbfb6`（UTC 04:12）两个油管号一起 404。频道还在，本机后来 `200`。
- **根因**：官方 `feeds/videos.xml` 会集体间歇 404/500，尤其 GitHub Actions 云 IP。采集只请求一次，失败就把成员标失败。
- **修法**：油管官方 RSS 404/429/5xx 隔 2 秒再试，最多 3 次；仍失败则从归档留下该频道上一轮油管条目，`fetch_mode=keep_last_rss`，`ok=true`。其它 RSS 不重试。不换 RSSHub。
- **Git**：`a3b93ec` 已合 `master`。NUC `C:\AI-news-reader\ai-news-radar-run` 已快进到含该提交的快照 `3f9cd7e`。
- **验收**：定向 OPML/油管测试 6 绿。生产 Actions `34227157604` 成功；小岛、脑总 `ok=true`、`fetch_mode=live_rss`、`keep_last_restored=0`（当轮没抽风）。用户要求合主线并部署生产。
- **当前无活跃 SPEC/PLAN/TASK/TEST**。轻量热修，无四文件。
- **未做**：留旧行/重试的真 404 现场。连续 1～2 天还不恢复才另开备用。
...
- **Git**：PR #37 `832fcdd` 已合 `master`。工作台嵌页戳另仓 PR #48，`wb=continue-watch-0830a`。
- **验收**：P5「统一时间口径」与「继续看剩余条目时不整表重画、不跳回顶部」通过。全量 e2e 67 条中 2 条为 18080 启动竞态，单独重跑通过。P6 用户口头通过：点剩余条目就地展开，不跳顶部。
- **当前无活跃 SPEC/PLAN/TASK/TEST**。轻量修复，无四文件。
- **未做**：无。
...
- **Git**：PR #36 `131c4ec` 已合 `master`。工作台嵌页戳另仓 PR #47，随后 PR #48 换成 `continue-watch-0830a`。
- **验收**：P5 `TEST-024` 与 TEST-018/019/021/022 通过。全量 e2e 15 红为油管订阅过滤 vs 旧夹具 81 条，本轮未改那些期望；夹具已由 continue-watch PR #37 对齐。P6 用户口头通过。NUC 生产仓已含 #36。
- **当前无活跃 SPEC/PLAN/TASK/TEST**。轻量修复，无四文件。
- **未做**：无（81 条断言已由 PR #37 恢复）。
