...
- Douyin maintenance now has a fixed local action to start `E:\AI-news-reader\MediaCrawler-local-test` in MediaCrawler creator mode through `scripts/run_mediacrawler_douyin.py`. The runner launches or reuses a dedicated Chrome CDP profile under `MediaCrawler-local-test\chrome-profile` instead of attaching to the user's everyday browser. The local collection panel now shows a plain-language Douyin collector progress card with running/completed state, JSONL line count, latest output time, and the next action. The normal radar refresh still only reads JSONL files; generated dated JSONL files are resolved by newest `creator_contents_*.jsonl` in the configured folder so old date paths do not keep the radar stuck on stale data.
