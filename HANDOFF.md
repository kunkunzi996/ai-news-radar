...
**「本轮采集哪些源」被当成了「历史里允许留哪些源」。** 这是两件事。

`cli.py` 原先用 `active_source_ids`（这轮要抓的源）去过滤 `archive`（历史归档）。于是线上和本机
各读各的名单、各跑各的，**互相把对方独有的源从同一份 `archive.json` 里删掉**。乒乓球式互删。

施工前用两份真实配置做纯计算模拟，实锤：

```
线上 Actions 跑一次   → 删 0 条
本机点一次「一键采集」→ 删 174 条（抖音34 + 微信62 + RSS78 当场清空）
```

2026-07-12 丢的那 9 条 GitHub Release，是同一颗雷反着炸了一次。

### 本轮做了什么

| # | 改动 | 关键点 |
|---|---|---|
| 1 | 采集管线不再按本轮范围裁剪归档 | 改用 `load_archive_for_collection()`；`--bilibili-only` / `--source-scope` 走同一行代码，同样会误删，一并解决 |
| 2 | 两份名单合一 | 本机刷新与 Actions 同读 `config/online-sources.json`；`sources.config.json` 退为本机私有参数 |
| 3 | 线上面板保存时也清历史 | 原先只有本机那份配置有 purge |
| 4 | 停用 = 取消订阅 | 见下方陷阱 A |
| 5 | RSS 源可清理 | 见下方陷阱 B |
| 6 | 采集占锁时记账，结束后补做 | 台账 `data/pending-purge.json`（已 gitignore） |
| 7 | 前端如实告知清理结果 | `utils.js` 的 `purgedItemsNote()`，三个保存入口共用 |

### ⚠️ 三个「照直觉改就会删掉不该删的数据」的陷阱

**陷阱 A —— 「停用即删除」不能只排除 current。**
Codex review 建议「比较身份时排除**当前**已停用的信源」。照做的话，`sources.config.json` 里
20 多个**长期处于 `off`** 的源（它们是「这通道本机不跑」，不是「取消订阅」）会在 previous 里
存在、current 里消失 → **被当成刚刚取消订阅**。实测：本机配置**什么都不改、原样保存一次**，
就会清掉抖音「Simon林」和小红书「陈抱一」的历史。
**正确做法：previous 和 current 两边都排除 disabled**，只有「上一版启用、这一版停用」才算取消。

**陷阱 B —— 加 `opmlrss` 进清理白名单前，必须先修身份映射。**
`SOURCE_CONFIG_TYPE_SITE_IDS` 里**没有 `"rss"` 这个 key**，所以 `type: rss` 的记录认出的
site_id 是**空集** → 存活名单为空 → **归档里每一条 RSS 都成了孤儿**。实测：直接把 `opmlrss`
加进 `PURGE_TRACKED_SITE_IDS`，**78 条 RSS 里被清掉 78 条，全灭**。
**顺序不可颠倒**：先让 `type: rss` 映射到 `opmlrss`（身份用配置 `name`，容器型 `type: opmlrss`
是「订阅包」不是订阅对象、不进身份表），确认存活名单是那 9 个真实源名，**再**加白名单。

**陷阱 C —— 延后清理必须有「加回来就不清」的安全阀。**
删了 A → 记进台账 → 采集还没结束 → 用户又把 A 加回来了。此时照台账清就会删掉一个**正订阅着**
的源的历史。补做前必须拿当前配置复核：台账里的源名，只有**现在确实不在配置里**的才清。
（已实测：台账记着「中二的大暄哥」，但他还在配置里 → flush 拒绝下手，19 条完好。）

### 「算账」与「动手」分离（第 6 项的设计要点）

`purge_deleted_source_data()` 其实是两步，**只有第二步需要锁**：

- **算账**：`deleted_source_names_by_site(new, previous)` → 纯计算，不碰 `data/*.json`，**不需要锁**
- **动手**：按名单去 5 个数据文件里删条目 → 需要锁

所以保存的那一刻就把账算完、把「被删的源名」记进台账。后面补做时只要这份名单，
**不再需要 `previous_config`**——那个「推迟到采集结束、旧配置早丢了怎么办」的难题自己就没了。

补做挂在 `refresh.py` 的 `finally` 里、**`REFRESH_LOCK.release()` 之前**（此时锁还在手里，
不会和下一次保存抢）。放在 `finally` 意味着采集失败/超时也照样还账。

### 数据结果

```
归档 565 → 560
  清除: 4 个 RSS 僵尸源共 20 条（Wired AI / 宝玉 / NVIDIA / InfoQ CN，早已不在任何 OPML 里）
  回填: GitHub Release 10 条
  9 个在订 RSS 源、B站 391、微信 62、抖音 34 —— 一条未损
```

首采临时用 `FIRST_COLLECT_BACKFILL_DAYS=180` 回填，长期默认 60 天规则未改。

### 验收

- pytest **326 passed**；`npm run test:e2e` **3 passed**
- **端到端浏览器实测**（这是唯一能抓住陷阱 C 的手段）：采集进行中删除 B站 UP 主 → 界面显示
  「1 个已删除信源的历史数据将在本次采集结束后清理」→ 把该源加回 → 台账自动清空 →
  采集结束后其 19 条历史**一条未损**；0 控制台错误

### 下一轮可能的入口

- `flush_pending_purge()` 用**线上配置**算存活名单，但台账也可能来自 `/api/source-config`
  （本机那份）的保存，两份名单不同。**这个偏差是偏保守的**（最坏是「该清的没清」，不会误删），
  等本机那份配置彻底退休就自然消失。
- `sources.config.json` 已退为本机私有参数回退，但 `/api/source-config` 接口仍在。要不要彻底
  下线它，是个独立话题。

## 历史交接：信源配置迁入独立设置抽屉（2026-07-13）
...
- GitHub Actions 的 `Update data` 改为读取 `--source-config config/online-sources.json`。
- 本地后台新增 local-only API：
...
- `.\.venv\Scripts\python.exe -m py_compile scripts\local_server.py scripts\radar\server\online_sources.py`
- `node --check assets\js\dom.js assets\js\online-source-config.js assets\js\boot.js`
- `.\.venv\Scripts\python.exe -m unittest tests.test_local_server -q`：70 tests OK。
...
7. `scripts/radar/server/online_sources.py`
8. `scripts/local_server.py`
9. `assets/js/online-source-config.js`
10. `assets/js/boot.js`
11. `assets/js/dom.js`
