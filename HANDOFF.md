...
- PR #44 已合 `master`。工作台嵌页 `wb=search-query-echo-0902a`（工作台 PR #55）。
...
- 已阅只按工作台链接键；网页 145 / App 142 用户已接受，不要当未修 bug。
- 改 JS 必须 bump `index.html` 的 `?v=`，工作台 iframe 必须换 `wb=`，否则嵌套页继续用旧脚本。
- 停采不等于清历史。禁止用采集范围扫 `archive.json`。
- 规格在工作台仓归档：`docs/archive/2026-08-30-App我的订阅对齐网页并停采公开博客/`。本仓无活跃四文件。
