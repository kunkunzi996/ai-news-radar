...
它更像是一条轻量的新闻pipeline，把来源判断、抓取、去重、AI相关性打分（可关闭）、信息源健康状态和静态网页发布串起来，上线后不消耗模型额度。
...
- `data/latest-24h.json`：最近24小时经 AI 相关性筛选后的消息（`AI_RELEVANCE_THRESHOLD=0` 时等于全量）
- `data/latest-24h-all.json`：最近24小时全量消息（「我的订阅」读这个）
...
当前高级配置直接保存到项目根目录的 `sources.config.json`，“刷新看板数据”按钮会
先写入配置，再按页面选择的“采集范围”触发一次固定的本地刷新脚本；它不会
...
微信公众号源已从默认云端输出和默认信源目录移除。旧 WeWe RSS / 猫笔刀备份代码
仅保留给本地显式实验，不再作为默认采集能力维护。
