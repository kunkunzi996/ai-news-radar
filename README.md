...
当雷达由本机 `万有 OMNIA` 工作台（`http://127.0.0.1:8765`）以 iframe 打开本机版
`http://127.0.0.1:8080/` 时，卡片操作区会出现“收藏”。点击后会把标题、链接、摘要和来源交给工作台收藏库，并同时标记该条已阅。

独立打开雷达（包括新标签页和 GitHub Pages）不会显示收藏按钮，也不会向工作台发请求；这是为了避免公开页面出现无效入口。工作台的“公网版”需要在包含该桥接代码的 Pages 部署完成后才会显示收藏按钮。
...
- 远程模式下「刷新看板数据」刷的是 NUC 本机数据；订阅变更后线上内容更新由
  「同步到线上」推送所触发的 Actions 完成。采集任务仍由 NUC 执行；微信管理后台已支持
  跨设备打开 `https://wechat.wanyouomnia.cn`，登录态失效时可在该页面扫码恢复。抖音、
  小红书等其他需要本机登录态的维护动作仍需回到 NUC 操作。
...
V3 已于 2026-07-16 完成真实账号验收。本地控制台仍负责首次绑定、收编已有手动源和解绑；
这些操作都必须先 Preview，再由用户确认后 Apply。

已绑定账号的星标会在每轮 Actions 采集前自动同步（约每 30 分钟）：新星标会自动接管；取消星标在
两个不同运行都确认缺失后才停用。空快照、数量骤降、账户/分页异常或重复 repo ID 都会熔断并转人工。
同步状态见 `data/github-star-autosync.json`，连续确认状态见 `data/github-star-purge-state.json`；没有配置
变化时，配置文件和 `updated_at` 保持不变，但状态心跳仍会刷新。

需要清理已经确认取消星标的 GitHub 历史时，先保持 `STAR_SUBSCRIPTION_CLEANUP_MODE=off` 等待两次
完整同步；然后由维护者手动设为 `audit`，核对 `data/github-star-subscription-cleanup.json` 的候选 repo ID、
`record.id` 与审批摘要。只有审核无误后才临时设置：

```text
STAR_SUBSCRIPTION_CLEANUP_MODE=on
STAR_SUBSCRIPTION_CLEANUP_APPROVAL_DIGEST=<本轮 audit 输出的摘要>
```

执行完成后立即把模式改回 `off` 并清空审批摘要。摘要失配时不得重试 `on`，应回到 `audit` 重新审核。
