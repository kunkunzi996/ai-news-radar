...
已绑定账号的星标会在每轮 Actions 采集前自动同步（约每 30 分钟）：取消星标会自动停用
对应源（历史保留，按 `archive_days` 自然老化），新星标会自动接管。空快照或数量骤降会
熔断并转人工处理。同步状态见 `data/github-star-autosync.json`；没有配置变化时，配置文件和
`updated_at` 保持不变，但状态心跳仍会刷新。
