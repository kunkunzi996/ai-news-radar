...
> **三条核心红线仍在 `CLAUDE.md` 正文**（必须触发计划任务且任务只保留抖音 action、
> 只在同步推送成功后派发、若恢复微信则只能全量重采）；本文装七条逐条契约与它们的实测依据——
...
4. **`DouyinCollectAndPush` 只保留抖音 action。** 2026-08-22 微信采集已下线；不要再加微信
   action（会重新跑已停用的采集链）。旧规则「必须保留抖音、微信两条」作废。运维说明见
