...
| GitHub starred repositories (local sync) | V3 code complete; real-account acceptance pending | Use the local console's Preview -> confirmation -> Apply flow | One numeric account id, up to 50 public stars; the 51st aborts. Private repositories are skipped without exposing identity. Releases are preferred; commit fallback keeps one newest snapshot per stable repository per UTC day. |
