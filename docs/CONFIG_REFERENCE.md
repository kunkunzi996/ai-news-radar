...
**想让自媒体/研究 Top 3 更满**:可调低门槛 `BRIEF_SCORE_GATE`。读者页已不再渲染伯乐精选 / Top 3（2026-09-09）；前端入口是 `assets/js/boot.js` 与 `assets/js/render-list.js`，不是旧的 `assets/app.js`。
...
## 12. 2026-07 新增的环境变量(桥接源与 AI 开关)

> ⚠️ 本文第 1 节的「每 30 分钟」等描述已过时(实际是每小时第 7、37 分),其余章节也未必反映最新代码,
> 以代码常量为准。本节只收录 2026-07 新增、且**必须知道**的变量。

### AI 相关性开关(决定主榜是否过滤)

| 变量 | 位置 | 当前值 | 含义 |
|---|---|---|---|
| `AI_RELEVANCE_THRESHOLD` | GitHub Actions **Variables** | **`0`** | AI 相关性阈值。缺省 `0.65`;设为 `0` = **不过滤**,主榜等于全量。项目已转向个人订阅聚合器,线上刻意关闭。**删掉这个变量即可恢复默认筛选。** 代码见 `scripts/ai_relevance.py` 的 `_load_ai_relevance_threshold()`。 |

### 微信公众号桥接(WeRSS)

| 变量 | 位置 | 含义 |
|---|---|---|
| `WECHAT_BRIDGE_REPO` | Actions **Variables** | 私有桥接仓库,当前 `kunkunzi996/wechat-bridge` |
...
| 变量 | 位置 | 含义 |
|---|---|---|
| `DOUYIN_BRIDGE_REPO` / `DOUYIN_BRIDGE_SSH_KEY` | Variables / Secrets | 同上,对应 `kunkunzi996/douyin-bridge` |
| `MEDIACRAWLER_LOCAL_DIR` | workflow 内部注入 | Actions 克隆抖音桥接后的 JSONL 目录 |

## 13. 线上信源同步合并与冲突提示

本机“同步到线上”发现云端 `config/online-sources.json` 已推进时，不会直接覆盖云端；它会以共同基线 B、本机候选 L、云端 R 为输入，按稳定的 `source.id` 生成合并结果 M。成功后先将 M 推送到远端，再把本机信源文件从 L 一步检出为 M。

可以自动合并的变更：

- 云端新增、停用或恢复 GitHub 星标受管信源，而本机修改了不同 `source.id`。
- 两端修改同一信源的不同字段，或把同一字段改成完全相同的值。
- 云端只有 `data/**` 快照推进时，继续沿用既有安全同步路径。

会停止同步并在设置页逐条显示冲突的情况：

- 两端对同一 `source.id` 的同一字段给出不同值。
- 一侧删除信源，另一侧修改该信源（`delete_vs_modify`）。
- `github_star_sync` 绑定、顶层 `version` 或 `mode` 不一致。
- 配置、派生 OPML、三方基线或 Git 事务证明无法验证。

冲突提示只包含信源名/ID、字段和经脱敏的本机/云端值；不会返回整份配置、locator 或凭证。合并成功时页面会显示“已自动合并云端 N 项变更”。同步合并不会触发历史清理或强制覆盖；云端删除信源时，本机 archive 历史仍保留。

---

*改完代码后,跑一遍全量测试确认无误:`.\.venv\Scripts\python.exe -m pytest -q`(基线 284 passed)*
