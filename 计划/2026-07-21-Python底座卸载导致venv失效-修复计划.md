...
- 状态：完成。
- 实际修改：仅更新本节施工报告；未改业务代码、环境目录、服务进程或计划任务。
- 实现说明：按端口反查与 `local_server.py` 命令行补漏取并集。为规避已接受的 PLAN-01 已知风险，排除了本次施工 PowerShell（PID 25676）及其祖先进程；未排除任何面板相关进程。
- 基线证据：8080 的底座子进程为 PID 23396（父 PID 32368）；8093 的底座子进程为 PID 7268（父 PID 38144）、PID 21572（父 PID 6136），命令行补漏还发现 PID 28688（父 PID 37604）。待停 PID 并集为 `6136, 7268, 21572, 23396, 28688, 32368, 37604, 38144`，同时覆盖全部 stub 父进程与 Python 3.11 底座子进程。`http://127.0.0.1:8080/` 和 `http://127.0.0.1:8093/` 均返回 HTTP 200。
- 与计划的差异：在待停清单生成时增加施工执行器及祖先进程排除保护，避免 T1 自行终止；其余流程与计划一致。
- 风险和遗留：T1 将连续停掉上述 8 个进程，8080/8093 会暂时不可用；T1 至 T6 之间不能中断。
- 下一步：执行 T0.5，临时停用 `DouyinCollectAndPush`，并在 T7.5 强制恢复。

### T0.5 — 停用采集计划任务

- 状态：完成。
- 实际修改：计划任务 `DouyinCollectAndPush` 已从 `Ready` 切换为 `Disabled`；未修改任务定义、触发器或动作。
- 实现说明：按冻结计划仅停用这一项同时承载抖音和微信采集的任务；`WechatHealthWatchdog` 保持启用。
- 与计划的差异：无。
- 测试与运行证据：`Get-ScheduledTask -TaskName 'DouyinCollectAndPush'` 返回 `State = Disabled`。
- 风险和遗留：停用期间两条采集链路都不会自动运行；T7.5 必须恢复为 `Ready` 且确认未来 `NextRunTime`。
- 下一步：进入已获用户授权的 T1，不可用窗口开始后连续完成 T2 至 T6。

### T1 — 停止服务并隔离旧环境

- 状态：完成。
- 实际修改：停止了 T0 记录的 8 个面板相关父子进程，并将 `.venv` 改名为 `.venv.broken-py311`；未删除任何目录或文件。
- 实现说明：待停 PID 为 `6136, 7268, 21572, 23396, 28688, 32368, 37604, 38144`。其中 PID 21572 在停止前已自行退出；其余 7 个 PID 均已停止。停止后端口反查和命令行补漏均为空，双重闸门一次通过后才执行改名。
- 与计划的差异：沿用 T0 的执行器排除保护；无其它差异。
- 测试与运行证据：`GatePassed = true`；8080/8093 无监听进程，无 `local_server.py` 残留进程；`.venv` 不存在，`.venv.broken-py311` 存在。
- 风险和遗留：面板服务已进入计划内不可用窗口。
- 下一步：连续执行 T2。
...
- 状态：完成。
- 实际修改：在原路径创建新的 `.venv`。
- 实现说明：执行 `py -3.13 -m venv .venv`，未安装或卸载系统 Python。
- 与计划的差异：无。
- 测试与运行证据：`.venv\\pyvenv.cfg` 指向 `C:\\Program Files\\Python313`；`.venv\\Scripts\\python.exe -V` 输出 `Python 3.13.14`；旧 `.venv.broken-py311` 仍保留。
- 风险和遗留：无新增风险。
- 下一步：执行 T3。
...
- 状态：完成。
- 实际修改：仅向新 `.venv` 安装 `requirements-dev.txt` 中锁定的依赖。
- 实现说明：按计划运行 `.venv\\Scripts\\python.exe -m pip install -r requirements-dev.txt`，未升级钉版、未使用 `--force-reinstall`。
- 与计划的差异：无。
- 测试与运行证据：pip 无 ERROR；`import requests, feedparser, bs4, dateutil, yaml, pytest` 输出 `ok`。核心版本为 requests 2.32.3、beautifulsoup4 4.12.3、feedparser 6.0.11、pytest 8.3.4、PyYAML 6.0.2、pyflakes 3.2.0。
- 风险和遗留：无新增风险。
- 下一步：执行 T4。
...
- 状态：部分完成，已触发 T5B 条件。
- 实际修改：无。
- 实现说明：新环境下健康探针可正常运行，随后执行两组看门狗测试。
- 与计划的差异：无；但 T0.5 执行前，23:00 已有一次旧环境采集失败，因此探针没有返回预期 `healthy`。
- 测试与运行证据：探针退出码 0，输出合法 JSON，`decision = alert`、`reason = pipeline_failed`。正式状态文件记录 `state = failed`、`stage = export`、`exit_code = 1`、`finished_at = 2026-07-21T15:00:41.9524448Z`，失败原因为 WeRSS JSONL export exit 103；计划任务最后启动时间为 23:00:06，早于本轮 T0.5。`.venv\\Scripts\\python.exe -m pytest tests\\test_wechat_health_probe.py tests\\test_wechat_health_watchdog.py -q` 通过：`36 passed in 37.00s`。
- 风险和遗留：`pipeline_failed` 使 T5B 的条件成立；T5B 需要用户对真实采集和 bridge 推送的单独授权。
- 下一步：T5 已可执行；T5B 等待授权。

### T5 — 无副作用采集链路诊断

- 状态：完成。
- 实际修改：仅在 TEMP 生成诊断 JSONL、状态文件和日志；未修改正式采集状态、bridge 工作区或提交历史。
- 实现说明：执行 `collect-wechat-and-push.ps1 -SkipSync`，使用 TEMP 状态文件 `C:\\Users\\Administrator\\AppData\\Local\\Temp\\t5-verify-status-20260722-003943.json`。
- 与计划的差异：按 T5 任务正文和脚本真实契约，传入 TEMP `-StatusFile`，以退出码 1 作为成功信号；未覆盖正式 `wechat-collect-status.json`。
- 测试与运行证据：退出码 1；状态为 `warning`、阶段为 `skip_sync_diagnostic_only`、状态文件 `exit_code = 1`；输出 60 条公开文章诊断数据；bridge HEAD 前后均为 `d8b7a8161ec984349cfb8685ef16d582ab116d91`，`git status --short` 前后均为空。运行期间出现 `MarkupResemblesLocatorWarning`，未导致失败。
- 风险和遗留：T4 的 `pipeline_failed` 仍未复位，必须在 T5B 真采集成功后才能继续 T6/T7。
- 下一步：等待用户明确批准 T5B 真采集（会写正式状态文件并向 `wechat-bridge` commit/push）。

### T5B — 手动补跑真实采集并推送 bridge

- 状态：完成。
- 实际修改：按正常采集链路更新正式 `E:\AI-news-reader\wechat-collect-status.json`，并在 `E:\AI-news-reader\wechat-bridge` 产生并推送提交 `7938941 数据：更新微信公众号公开文章与订阅快照`；未修改业务代码、计划任务定义或归档清理开关。
- 实现说明：T4 已确认 `alert / pipeline_failed`，且 T5 已按诊断路径契约通过；收到产品负责人对真实采集和 bridge 推送的明确授权后，按计划不带 `-SkipSync` 执行一次受控真跑。
- 与计划的差异：无。计划正文中 T5 的旧 `-StatusFile` 文字与实际契约冲突仍作为已接受的 PLAN-02 风险保留，未修改 ①/②；本任务自身按要求传入正式状态文件。
- 测试与运行证据：进程退出码 `0`；正式状态为 `succeeded / completed_pushed / exit_code=0`，输出公开文章 `60` 条，`failed_creator_count=0`，`new_unique_items=0` 按计划不作失败判据。bridge HEAD 由 `d8b7a8161ec984349cfb8685ef16d582ab116d91` 变为 `7938941fc015b39885c85b9d409e41243f01acbc`，`git log --oneline -3` 可见新提交，bridge 工作区已与 `origin/main` 同步且无未提交修改。
- 风险和遗留：运行中出现既有 `MarkupResemblesLocatorWarning`，未导致采集、状态写入或推送失败；正式采集日志仍保留本轮之前的历史错误记录，不能将历史日志文本误判为本次失败。
- 下一步：进入 T6，恢复 8080 面板并验证四个 worktree。

### T6 — 恢复服务并逐一验证

- 状态：完成。
- 实际修改：通过既有启动器恢复 8080 面板；未恢复 8093，符合冻结计划“仅恢复 8080、8093 另由用户决定”的边界。
- 实现说明：T5 与已触发的 T5B 均按各自退出码契约通过后，执行 `scripts\windows\open-ai-news-radar.ps1 -NoBrowser` 恢复默认面板端口。
- 与计划的差异：无。
- 测试与运行证据：四个 worktree 的 `.venv\Scripts\python.exe -V` 均为 `Python 3.13.14`；`http://127.0.0.1:8080/` 返回 HTTP 200，当前内容长度 25731 且含信息流内容。已用真实浏览器完成验收：页面正常渲染、有信息流条目、4/4 信源正常、控制台无 error。
- 风险和遗留：8093 未恢复是计划内选择，不代表 8080 面板故障。
- 下一步：进入 T7，复位看门狗告警状态。

### T7 — 看门狗端到端恢复

- 状态：完成。
- 实际修改：看门狗将事故状态恢复为正常，并发送一次恢复通知；未修改采集数据或任务定义。
- 实现说明：在正式采集状态已为 `succeeded` 后执行看门狗。首次运行记录 `recovery_sent`，随后复核运行记录 `healthy`，因此未重复推送。
- 与计划的差异：无。
- 测试与运行证据：`wechat-watchdog.log` 包含 `[2026-07-22 00:44:53] recovery_sent` 和 `[2026-07-22 00:45:19] healthy`；`wechat-watchdog-state.json` 为 `status=ok`、`primary_reason=ok`；`wechat-watchdog-run-status.json` 为 `succeeded / healthy / exit_code=0`。
- 风险和遗留：脚本已成功调用恢复通知通道；终端设备的实际到达确认仍由产品负责人观察。
- 下一步：执行强制回收项 T7.5，恢复采集计划任务。

### T7.5 — 恢复采集计划任务

- 状态：完成。
- 实际修改：将 `DouyinCollectAndPush` 从临时停用恢复为启用；未修改任务定义、触发器或动作。
- 实现说明：T0.5 与 T7.5 成对执行，避免采集长期停摆。
- 与计划的差异：无。
- 测试与运行证据：`Get-ScheduledTask` 返回 `State = Ready`；`Get-ScheduledTaskInfo` 返回 `NextRunTime = 2026-07-22 10:00:00 +08:00`（未来时刻）且 `LastTaskResult = 0`。
- 风险和遗留：无新增风险。
- 下一步：进入 T8 用户侧收尾与代码审查/QA 验收。

### T9 — 产品负责人授权的 MediaCrawler 独立环境修复尝试

- 状态：**BLOCKED**。
- 授权与范围：`QA-03` 发现抖音动作使用独立仓库 `E:\AI-news-reader\MediaCrawler-local-test` 的旧 Python 3.11 环境后，产品负责人明确授权本次只修复该 `venv` 并回归原路径；不改业务代码、配置、登录态、桥接数据或计划任务定义，保留该仓库已有的三项未提交源码改动。
- 实际修改：旧 `venv` 已改名保留为 `venv.broken-py311`；成功创建过 Python 3.13.14 的同名 `venv`，但依赖安装失败后按失败处置改名保留为 `venv.failed-py313-20260722-105546`。未删除任何文件或目录。
- 实现说明：重建前排除了施工器自身并确认没有正在运行的 MediaCrawler Python 进程；Chrome CDP 登录配置保持运行、未关闭。新环境 `pyvenv.cfg` 曾确认指向 `C:\Program Files\Python313`。
- 与计划的差异：原冻结计划只覆盖 4 个 Radar worktree；本任务是产品负责人针对 `QA-03` 授权的独立环境扩展。依赖版本升级属于新的兼容性方案，不在本次授权范围，未执行。
- 测试与运行证据：`py -3.13 -m venv venv` 成功，`python -V` 为 `Python 3.13.14`；按仓库原 `requirements.txt` 安装时，`playwright==1.45.0` 的 `greenlet` 在 Python 3.13 编译失败（`pycore_frame.h` 报“this header requires Py_BUILD_CORE define”），`pydantic==2.5.2` 依赖的 `pydantic-core==2.14.5` 亦因 `ForwardRef._evaluate()` 接口不兼容失败。未执行真实抖音采集，未产生 bridge 提交。
- 风险和遗留：`MediaCrawler-local-test` 现有 `config/base_config.py`、`config/dy_config.py`、`media_platform/xhs/playwright_sign.py` 三项改动保持原样；两个 venv 留档均为未跟踪环境目录，不得提交或删除。`DouyinCollectAndPush` 的抖音动作仍无可运行解释器。
- 下一步：需产品负责人决定是否为该独立仓库重开 Python 3.13 依赖兼容计划（更新受影响依赖并跑抖音/小红书回归），或明确采用其它运行时策略；本轮到此暂停。

### T10 — 独立 MediaCrawler 的 Python 3.13 兼容性修复与自动任务复核

- 状态：完成（保留风险）。
- 授权与范围：产品负责人已确认按独立兼容性修复方案继续。先在干净工作树 `E:\AI-news-reader\MediaCrawler-py313-compat`、分支 `fix/python313-compat` 验证，再把同一套已验证的依赖声明同步回计划任务硬编码使用的 `E:\AI-news-reader\MediaCrawler-local-test`；不覆盖该目录既有的三项源码改动，不改登录态、采集任务定义或 bridge 业务代码。
- 实际修改：两个 MediaCrawler 工作树均将 `.python-version` 从 `3.11` 改为 `3.13`；`requirements.txt` 和 `pyproject.toml` 中仅将 `playwright` 从 `1.45.0` 升至 `1.48.0`、`pydantic` 从 `2.5.2` 升至 `2.10.0`；由 `uv lock` 受控更新 `uv.lock`，仅连带更新 `greenlet 3.0.3 → 3.1.1`、`pydantic-core 2.14.5 → 2.27.0`、`pyee 11.1.0 → 12.0.0`。原目录新建可运行的 `venv`（Python `3.13.14`）；旧 `venv.broken-py311` 与失败留档 `venv.failed-py313-20260722-105546` 均保留，未删除。
- 实现说明：根因是旧 `greenlet`、`pydantic-core` 没有可用于 Python 3.13 的兼容构建，不是采集代码本身。选择首个可用的最小版本组合，避免升级其它直接依赖；计划任务仍从原路径读取 `venv\Scripts\python.exe`，因此不改任务定义而在原路径完成受控部署。
- 与计划的差异：T10 是 T9 明确要求的产品负责人授权后独立兼容性方案，超出原冻结计划的四个 Radar worktree 范围；未修改 ①/②，也未将原目录已有的 `config/base_config.py`、`config/dy_config.py`、`media_platform/xhs/playwright_sign.py` 纳入本轮改动。
- 测试与运行证据：隔离工作树及原目录均通过 `pip check`；原目录版本为 Python `3.13.14`、Playwright `1.48.0`、Pydantic `2.10.0`、Greenlet `3.1.1`、Pydantic Core `2.27.0`。`python -m pytest -q tests/test_cdp_browser.py tests/test_api_limits.py` 为 `13 passed`。完整 pytest 为 `61 passed, 8 skipped, 7 failed`：其中 6 项需要本机 Redis 正确密码，另 1 项为未触及的 XHS Excel 存储工厂断言；均未被隐藏或修改。真实最小回归：抖音 `4/4` 博主完成、失败 `0`、登录正常；小红书登录正常并成功写入 1 条 JSONL。随后人工触发完整 `DouyinCollectAndPush`：抖音状态 `completed_pushed / exit_code=0 / 4/4 完成 / 0 失败`，bridge 新提交 `eb3fcaf 数据：更新抖音采集 JSONL`；微信状态 `completed_no_change / exit_code=0`；任务最终 `Ready`、`LastTaskResult=0`、下一次为 `2026-07-22 15:00:00 +08:00`。
- 风险和遗留：CDP 连接会先尝试旧 WebSocket 地址并收到 404 警告，随后自动经 `/json/version` 回退并成功，不影响本轮采集。完整 pytest 的 7 项外部环境/既有断言失败未修复；两个旧 venv 留档及独立兼容分支/工作树均待产品负责人决定后续版本保存与收尾，禁止批量删除。
- 版本保存补充：产品负责人已明确确认保存版本；兼容分支 `fix/python313-compat` 已提交 `cd2f303 修复：兼容 Python 3.13 采集环境`，仅含四个受控依赖文件。提交后工作树干净；未 push、未合并、未改动原目录已有的三项源码修改。
- 下一步：QA 已完成真实验收；等待产品负责人决定是否 push / 合并该专用分支，以及何时按 T8 手动处理旧 venv 留档。

### 整轮结论

**COMPLETE WITH RISKS**。冻结计划中的 T0、T0.5、T1-T7.5 已执行；T9 的独立环境阻塞已由产品负责人授权的 T10 兼容性修复闭环。Radar 业务代码改动为 0，独立 MediaCrawler 仅有受控依赖声明与锁文件改动；Python 3.13 环境、抖音/小红书采集、两个 bridge、面板、看门狗和采集计划任务均已实际验证。保留的真实遗留为：`.venv.broken-py311`、`venv.failed-py313-20260722-105546` 和旧 Python 3.11 卸载残留须由产品负责人按 T8 在观察 1-2 天后手动处理；完整 pytest 的 7 项环境/既有失败、CDP 回退警告、`MarkupResemblesLocatorWarning` 与历史日志错误需后续观察；恢复通知的终端到达已由产品负责人确认。
...
### 第一轮代码审查 · CODE_REVIEW（2026-07-22）

- 审查范围：真实 diff、① 冻结计划、③ 施工报告，以及新环境、计划任务、状态文件和 bridge 的只读复核。业务代码 diff 为 0；当前受审的受控文本改动仅为本计划书的施工报告。
- 已复核：`.venv\pyvenv.cfg` 指向 `C:\Program Files\Python313`，四个 worktree 的 `.venv` 均为指向主 `.venv` 的 Junction；正式采集状态为 `succeeded / completed_pushed / exit_code=0`，bridge `7938941` 已与 `origin/main` 同步；看门狗状态为 `ok`，独立健康探针返回 `healthy / ok`；`DouyinCollectAndPush` 为 `Ready`。
- 自动验证：`.venv\Scripts\python.exe -m pytest tests\test_wechat_health_probe.py tests\test_wechat_health_watchdog.py -q` 结果 `36 passed in 36.60s`；`git diff --check` 退出码 0（仅有 CRLF 转换提示）。

#### CODE-01 · P2 · OPEN（非阻塞，需产品负责人确认风险）

- 文件与位置：① 计划正文第 257、470、547 行。
- 具体场景：冻结计划把跨过 23:00 后的下一轮写成“次日 07:00”，但已注册的 `DouyinCollectAndPush` 真实触发器是每天 10:00、15:00、23:00；本轮 `NextRunTime = 2026-07-22 10:00:00 +08:00` 是正确行为。继续按 07:00 观察会误判任务未恢复，或漏掉实际 10:00 的首轮复核。
- 最小修复要求：本轮不改冻结 ①；后续若复用本计划，先由产品负责人授权更新运行时点，并以计划任务真实触发器为准。

#### CODE-02 · P2 · OPEN（非阻塞，提交范围风险）

- 文件与位置：仓库根目录 `git status --short`。
- 具体场景：工作区除本计划书外还存在 `.venv.broken-py311/`、`.worktrees/`、补丁和其他未跟踪文档；若使用 `git add .` 或 `git add -A`，会把本轮未审查、甚至不应版本化的内容一起提交。
- 最小修复要求：提交前仅精确暂存本计划书路径；`.venv.broken-py311/` 按 T8 保留观察，不纳入提交。

**结论：APPROVE WITH RISKS**。未发现业务代码、数据迁移、权限、接口或环境重建链路的阻塞性问题；上述两项均不阻断进入真实验收。浏览器页面、恢复通知在终端设备的实际到达、以及下一次计划任务真实触发仍未在本模式重新执行，须在 `ACCEPTANCE_TEST` 中如实验收。

【下一步交给：代码审查官 / QA（ACCEPTANCE_TEST）；完成后交产品负责人终审】

### 真实验收 · ACCEPTANCE_TEST（2026-07-22）

- 验收范围：Python 3.13 环境修复后的本地面板、微信采集状态、看门狗恢复和采集计划任务恢复；未重新触发真采集、bridge 推送或任何数据清理操作。
- 原故障路径：通过新 `.venv` 启动的健康探针返回 `healthy / ok`；8080 当前监听进程命令行是 `E:\AI-news-reader\ai-news-radar-run\.venv\Scripts\python.exe scripts/local_server.py --host 127.0.0.1 --port 8080`，已不依赖已卸载的 Python 3.11。
- 自动化回归：`.venv\Scripts\python.exe -m pytest tests\test_wechat_health_probe.py tests\test_wechat_health_watchdog.py -q` 结果 `36 passed in 36.60s`。
- 浏览器验收（Playwright）：打开 `http://127.0.0.1:8080/` 后默认信息流正常显示 `716` 条订阅、`4/4 源正常`；切换“微信公众号”后显示 `83` 条、4 个来源和真实文章；刷新后页面重新正常渲染；两次控制台检查均为 `0` 个 error、`0` 个 warning。截图留存于 `.playwright-cli\page-2026-07-21T17-02-10-608Z.png`。
- 状态与回归：正式采集状态为 `succeeded / completed_pushed / exit_code=0 / output_rows=60 / failed_creator_count=0`；看门狗日志包含 `recovery_sent` 和后续 `healthy`，状态文件为 `ok`；`DouyinCollectAndPush` 为 `Ready`，下次自动运行时间为 `2026-07-22 10:00:00 +08:00`。
- 未适用：本轮是环境恢复，没有新增表单、数据写入、权限或网络重试逻辑，因此不以空输入、错误输入或重复点击作为验收对象；浏览器只做无副作用的栏目切换和刷新。

#### QA-01 · P2 · CLOSED（历史记录：10:00 前尚未实际运行）

- 文件与位置：Windows 计划任务 `DouyinCollectAndPush`。
- 具体场景：当前时间为 2026-07-22 01:02 +08:00，下一次自动触发在 10:00；本轮不手动启动该任务，以免在没有新授权时再次执行真实采集与 bridge 推送。因此“修复后首个自动触发”尚未被实际观察。
- 最小验证要求：10:00 任务结束后，复核 `wechat-collect-status.json` 为 `succeeded`、`exit_code=0`，并确认 bridge 没有异常提交或未推送状态。

#### QA-02 · P2 · 外部确认待补（非阻塞）

- 文件与位置：MeoW 恢复通知终端设备。
- 具体场景：看门狗日志已记录 `recovery_sent`，但本环境不能读取产品负责人的手机通知，因此无法独立证明通知已在终端设备显示。
- 最小验证要求：产品负责人确认已收到“微信采集已恢复”通知；未收到则保留日志和 `wechat-watchdog-run-status.json` 交回 Developer 排查。

**结论：ACCEPTED WITH RISKS**。核心修复路径、面板与健康链路已真实运行通过；两项 P2 都是外部时点/终端确认，不影响当前服务可用，但须按上述方式补观察。

【下一步交给：产品负责人终审】

### 验收补充（产品负责人确认）

- `QA-02`：**CLOSED**。产品负责人已于 2026-07-22 确认手机实际收到“微信采集已恢复”通知；看门狗日志中的 `recovery_sent` 与终端到达结果一致。
- 当前结论：仍为 **ACCEPTED WITH RISKS**，仅保留 `QA-01` 的时间性观察项：等待 10:00 自动任务实际运行后复核其正式状态。

【下一步交给：产品负责人终审】

### 10:00 自动任务复核（QA-01）

- 复核范围：仅读取 `DouyinCollectAndPush` 的实际运行记录、正式采集状态、采集日志、bridge 状态和健康探针；未手动触发采集、未重跑 bridge 推送、未修改任何业务代码或计划任务。
- 实际结果：计划任务于 `2026-07-22 10:00:05 +08:00` 自动启动，下一次为 `15:00:00 +08:00`。虽然 Windows 任务摘要显示 `LastTaskResult = 0`，但本轮的权威正式状态文件记录为 `warning / authority_unavailable / exit_code=1`，不能按任务摘要判定为采集成功。
- 外部依赖复核：8001 WeRSS sidecar 仍在 `127.0.0.1:8001` 监听且首页 HTTP 200；本轮三条公众号同步均报 `Invalid Session`，健康探针返回 `alert / login_expired`。bridge HEAD 保持 `7938941`，工作区与 `origin/main` 同步，无异常提交或未推送状态。

#### QA-01 · P1 · OPEN（10:00 实测后的状态升级）

- 文件与位置：Windows 计划任务 `DouyinCollectAndPush`；`E:\AI-news-reader\wechat-collect-status.json`；`E:\AI-news-reader\wechat-collect.log`。
- 具体失败场景：10:00 自动任务实际执行后，猫笔刀、数字生命卡兹克、卡尔的AI沃茨均返回 `Invalid Session`；正式状态为 `output_rows=0`、`failed_creator_count=3`、`bridge_changed=false`，因此本轮公众号自动采集没有完成。
- 根因判定：现有证据表明是微信登录态过期，非 Python 3.13 环境、8080 面板或 8001 sidecar 不可用；但不据此推断“重建环境导致登录失效”。
- 最小修复要求：产品负责人先在 `http://127.0.0.1:8001/` 完成微信重新扫码登录；随后保持计划任务为 `Ready`，等待 `15:00` 自动任务自然运行，再复核正式状态为 `succeeded / exit_code=0`，且 bridge 无异常未推送状态。未经新的明确授权，不手动补跑真实采集。

**验收结论：BLOCKED BY ENVIRONMENT。** Python 3.13 重建后的计划任务已确实被调度和执行，但当前缺少有效微信登录态，核心自动采集路径未通过；`QA-02` 仍保持 CLOSED。

【下一步交给：产品负责人完成扫码登录；完成后交 QA 复核 15:00 自动任务】

### 用户重新授权后的人工触发验证 · ACCEPTANCE_TEST（2026-07-22）

- 授权与范围：产品负责人完成微信重新授权后，明确授权立即执行一次真实自动采集。QA 通过 `Start-ScheduledTask -TaskName "DouyinCollectAndPush"` 触发已注册的原始计划任务；该任务会同时执行既有的微信与抖音动作。未修改任务定义、业务代码、环境目录或 bridge 文件。
- 运行证据：任务于 `2026-07-22 10:39:05 +08:00` 开始，完成后回到 `Ready`，下一次仍为 `15:00:00 +08:00`，任务摘要 `LastTaskResult = 0`。
- 微信结果：正式状态为 `succeeded / completed_no_change / exit_code=0 / login_state=valid / output_rows=60 / failed_creator_count=0`；`completed_no_change` 表示本轮无新公开数据，属于成功结果。健康探针退出码 `0`，返回 `healthy / ok`。wechat bridge HEAD 保持 `7938941`，与 `origin/main` 同步且无未提交改动，未产生 bridge 变更符合本轮无数据变化的预期。
- 看门狗观察：10:30 的看门狗早于本次成功采集，故仍记录此前失败的 `alerting / login_expired`；本次未额外手动触发看门狗或发送通知，交由下一次既有计划运行按新鲜成功状态自行恢复。

#### QA-01 · P1 · CLOSED（微信登录态恢复后的原路径复验）

- 文件与位置：Windows 计划任务 `DouyinCollectAndPush` 的微信动作；`E:\AI-news-reader\wechat-collect-status.json`；`E:\AI-news-reader\wechat-collect.log`。
- 原失败场景：10:00 自动任务的三条公众号均为 `Invalid Session`，正式状态为 `authority_unavailable / exit_code=1`。
- 复验结果：同一计划任务在产品负责人重新授权后于 10:39 真正执行完成，状态回到 `succeeded / exit_code=0`、登录态为 `valid`、失败创作者数为 0；10:00 已证明调度器按触发器自动启动，10:39 又证明其原始任务动作在新登录态下可成功执行。
- 最小修复要求：已满足；无需再次手动补跑微信采集。

#### QA-03 · P1 · OPEN（完整计划任务的抖音动作仍受旧 Python 底座阻断）

- 文件与位置：Windows 计划任务 `DouyinCollectAndPush` 的抖音动作；`E:\AI-news-reader\MediaCrawler-local-test\venv\pyvenv.cfg`；`E:\AI-news-reader\douyin-collect-status.json`。
- 具体失败场景：同一次 10:39 计划任务中，抖音动作在约 4 秒后失败，正式状态为 `failed / runner / exit_code=1`，底层 MediaCrawler 返回 `exit=103`，日志明确为 `No Python at 'C:\\Program Files\\Python311\\python.exe'`；douyin bridge HEAD 未变，未产生脏提交。
- 根因判定：`MediaCrawler-local-test\venv` 仍记录 `home = C:\\Program Files\\Python311`、`version = 3.11.9`，而该 Python 已卸载。该目录不属于冻结计划明确验证的四个 worktree，且其独立仓库当前已有 `config/base_config.py`、`config/dy_config.py`、`media_platform/xhs/playwright_sign.py` 三项未提交改动。
- 最小修复要求：**需重开计划**。由产品负责人单独授权该独立仓库的环境修复方案，先保护并核对上述现有改动，再将其 `venv` 重建为 Python 3.13 并验证一次抖音原路径；QA 不越权在该脏工作区重建、删除或提交任何内容。

**验收结论：REJECTED（针对完整 `DouyinCollectAndPush` 自动任务）。** 微信采集路径已恢复并关闭 `QA-01`，但同任务的抖音动作仍有一个阻塞性 P1；不能把“微信成功”表述为整项自动采集全绿。

【下一步交给：产品负责人决定是否为 `E:\AI-news-reader\MediaCrawler-local-test` 重开专项修复计划；微信链路可继续按既有 15:00 调度运行】

### T10 兼容性修复后的真实验收 · ACCEPTANCE_TEST（2026-07-22）

- 验收范围：只读审查独立兼容分支 `fix/python313-compat` 与原计划任务目录的依赖 diff，并复验原 `QA-03` 的“旧 Python 解释器不存在”失败路径；不重复启动采集、不重复推送 bridge、不改任何代码或环境。
- Diff 复核：本轮受控依赖改动仅为 `.python-version`、`requirements.txt`、`pyproject.toml`、`uv.lock`；两处工作树的上述四个文件 SHA-256 一致，`git diff --check` 通过。原目录另有 `config/base_config.py`、`config/dy_config.py`、`media_platform/xhs/playwright_sign.py` 三项既有改动，未纳入本轮 diff。
- 运行时复核：原目录 Python 3.13 `venv` 的 `pip check` 通过；`python -m pytest -q tests/test_cdp_browser.py tests/test_api_limits.py` 为 `13 passed in 0.57s`。审查时曾在外层目录错误调用相对 tests 路径，得到“file not found”；随后已在真实仓库目录复跑，不将该命令错误误记为产品测试失败。
- 原故障路径复验：人工触发后的完整 `DouyinCollectAndPush` 已从原路径的 Python 3.13 `venv` 实际运行。抖音正式状态为 `completed_pushed / exit_code=0 / requested=4 / completed=4 / failed=0`，douyin bridge 新 HEAD 为 `eb3fcaf` 且工作区干净；微信正式状态为 `completed_no_change / exit_code=0`，wechat bridge 工作区干净；Windows 任务为 `Ready`、`LastTaskResult=0`、下一次为 `15:00:00 +08:00`。

#### QA-03 · P1 · CLOSED（独立 MediaCrawler 的旧 Python 底座阻断已消除）

- 文件与位置：Windows 计划任务 `DouyinCollectAndPush` 的抖音动作；`E:\AI-news-reader\MediaCrawler-local-test\.python-version`、`requirements.txt`、`pyproject.toml`、`uv.lock`、`venv\pyvenv.cfg`。
- 原失败场景：10:39 的同一计划任务找不到 `C:\Program Files\Python311\python.exe`，导致抖音动作 `failed / runner / exit_code=1`。
- 复验结果：`venv\pyvenv.cfg` 现指向 `C:\Program Files\Python313`；依赖检查、CDP 定向测试和抖音/小红书真实回归均通过。11:18 的原始计划任务再次完整执行，抖音和微信两项均为退出码 0，抖音 bridge 已完成数据提交。
- 最小修复要求：已满足；无需等待 15:00 再判定本轮修复是否生效。

#### QA-04 · P2 · OPEN（完整 pytest 的外部环境/既有测试噪声）

- 文件与位置：MediaCrawler 测试套件中的 Redis 代理池测试与 XHS Excel 存储工厂测试。
- 具体场景：隔离工作树完整 pytest 为 `61 passed, 8 skipped, 7 failed`；其中 6 项需要本机 Redis 正确密码，另 1 项未触及的 XHS Excel 存储工厂断言失败。它们不影响本轮真实 CDP 采集和计划任务成功，但当前不能将完整测试套件描述为全绿。
- 最小修复要求：后续单开测试环境治理/存储工厂测试修复任务；本轮不修改 Redis 凭证、代理配置或无关存储逻辑。

**验收结论：ACCEPTED WITH RISKS。** 原 QA-03 的阻塞性 P1 已关闭；依赖 diff 范围受控，原任务已真实跑通。仅保留 QA-04 与版本保存/旧环境留档等非阻塞风险。
