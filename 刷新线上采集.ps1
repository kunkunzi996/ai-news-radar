...
<#
  ...
  三个入口：
    [1] 全部采集      本机抓抖音推桥接 -> 再触发云端聚合
    [2] 只刷云端      只触发 Actions，刷新 B站/RSS/GitHub/YouTube 直连源
    [3] 只采本机源    只抓抖音并推桥接仓库（备料，页面不变；跑完可再选是否触发云端）
...
Write-Host '  [1] 全部采集    本机抖音 -> 云端聚合'
Write-Host '  [2] 只刷云端    仅触发 Actions，刷 B站/RSS/GitHub/油管'
Write-Host '  [3] 只采本机源  仅抓抖音推桥接（备料，页面不变）'
Write-Host '  [Q] 退出'
